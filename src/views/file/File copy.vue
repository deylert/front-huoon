        ...
        <v-col cols="12" md="3" class="text-right">
          <v-dialog v-model="dialog" max-width="600px">
            <template v-slot:activator="{ props }">
              <v-btn v-bind="props" class="text-subtitle-1 ml-12" color="#DA7171" variant="flat" elevation="2"
            ...
            <v-card>
              ...
              <v-card-text>
                <v-container>
                  <v-row>
                    <v-col cols="12">
                      <v-text-field v-model="editedItem.title" clearable label="Título del Archivo"
                        prepend-icon="mdi-file" variant="underlined"></v-text-field>
                    </v-col>
                    <v-col cols="12" md="6">
                      <v-text-field v-model="editedItem.description" clearable label="Descripción del Archivo"
                        prepend-icon="mdi-pencil" variant="underlined"></v-text-field>
                    </v-col>
                    <v-col cols="12" md="6">
                    ...
                    <v-col cols="12" md="6">
                      <v-select v-model="editedItem.category" :items="categories" label="Categoría"
                        prepend-icon="mdi-folder" variant="underlined"></v-select>
                    </v-col>
                    <v-col cols="12" md="6">
                      <v-checkbox v-model="editedItem.isImportant" label="¿Es archivo importante?"
                        color="success"></v-checkbox>
                    </v-col>

                    <!-- Aquí agregamos el campo para cargar archivos -->
                    <v-col cols="12" md="6">
                      <v-file-input v-model="editedItem.file" label="Seleccionar archivo" prepend-icon="mdi-paperclip"
                        accept="application/pdf, image/*, video/*" variant="underlined"></v-file-input>
                    </v-col>
                  </v-row>
                </v-container>
              </v-card-text>
    ...
    <v-card-text>
      ...
      <v-data-table :headers="headers" :search="search" :items="files" class="elevation-1">
        <template v-slot:item.actions="{ item }">
          <v-btn density="comfortable" icon="mdi-pencil" @click="editItem(item)" color="#5D89E9" variant="tonal"
            elevation="1" title="Editar Archivo"></v-btn>
          <v-btn density="comfortable" icon="mdi-delete" @click="deleteItem(item)" color="red-darken-4" variant="tonal"
            elevation="1" title="Eliminar Archivo"></v-btn>
        </template>
      </v-data-table>
    </v-card-text>
  </v-card>
