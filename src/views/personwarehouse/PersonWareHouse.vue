<template>
    ...
    <v-card elevation="6" class="mx-2">
        <v-toolbar color="#03626C">
            <v-row align="center">
                <v-col cols="12" md="8" class="grow ml-4">
                    <span class="text-subtitle-1"><strong>Listado de Almacenes</strong></span>
                </v-col>
                <v-col cols="12" md="3" class="text-right">
                    <v-btn class="text-subtitle-1 ml-12" color="white" variant="tonal" elevation="2"
                        prepend-icon="mdi-plus-circle" @click="showAddWareHouse">
        ...
        <v-card-text>
            ...
            <v-data-table :headers="headers" :search="search" :items="personwarehouses" class="elevation-1"
                ...
                <template v-slot:item.actions="{ item }">
                    <v-btn density="comfortable" icon="mdi-pencil" @click="editItem(item)" color="primary"
                        variant="tonal" elevation="1" title="Editar Almacén"></v-btn>
                    <v-btn density="comfortable" icon="mdi-delete" @click="deleteItem(item)" color="#DA7171"
                        variant="tonal" elevation="1" title="Eliminar Almacén"></v-btn>
                </template>
                <template v-slot:item.status="{ item }">
                    <span v-if="item.status === 0" class="text-success">Público</span>
                    <span v-else class="text-error">Privado</span>
                </template>
            </v-data-table>
        </v-card-text>
    </v-card>
                        ...
                        <v-row>
                            <v-col cols="12" md="6" v-show="editedIndex === -1">
                                <v-autocomplete :no-data-text="'No hay datos disponibles'"
                                    v-model="editedItem.warehouse_id" :items="warehouses" label="Almacenes"
                                            ...
                                            <v-list-item-subtitle class="d-flex flex-column">
                                                <v-tooltip bottom>
                                                    <template v-slot:activator="{ on, attrs }">
                                                        <div class="description-text" v-bind="attrs" v-on="on"
    ...
    <v-dialog v-model="dialogDelete" max-width="500px">
        <v-card>
            <v-toolbar color="#DA7171">
                <span class="text-subtitle-2 ml-4"> Eliminar un almacén</span>
            </v-toolbar>
...
export default {
    data: () => ({
        ...
        data: {},
        headers: [
            { title: 'Nombre', value: 'title', width: '25%' },
            { title: 'Descripción', value: 'description', width: '25%' },
            { title: 'Ubicación', value: 'location', width: '15%' },
            { title: 'Estado', value: 'status', width: '10%' },
        ...
        async showAddWareHouse() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    this.warehouses = result.data?.warehouses?.filter(warehouse => 
                    !this.personwarehouses.some(personwarehouse => personwarehouse.warehouse_id === warehouse.id)) || [];
                } else {
